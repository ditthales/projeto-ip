...
class Player:
    ...
    def desenhar(self, tela):
        ...
        if self.is_walking_left:
            tela.blit(pygame.transform.scale(pygame.transform.flip(Player.player_walk_images[self.animation_count // 12], True, False),(self.largura,self.altura)),(self.x,self.y))
        elif self.is_walking_right:
            tela.blit(pygame.transform.scale(Player.player_walk_images[self.animation_count // 12],(self.largura,self.altura)),(self.x,self.y))
        else:
            tela.blit(pygame.transform.scale(Player.player_idle,(self.largura,self.altura)),(self.x,self.y))
